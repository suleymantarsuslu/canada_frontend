...
const Users = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get('https://api.canada-ankara.com/api/admin/users', {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editUser
        ? `https://api.canada-ankara.com/api/admin/users/${editUser._id}`
        : 'https://api.canada-ankara.com/api/admin/users';
  ...
  const handleDelete = (id) => {
    ...
    setConfirmModalAction(() => () => {
      axios
        .delete(`https://api.canada-ankara.com/api/admin/users/${id}`, {
          headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
        })
        .then(() => {
          fetchUsers();
